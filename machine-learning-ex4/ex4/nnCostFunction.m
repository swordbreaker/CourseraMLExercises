...
for k = 1:num_labels
    h = a3(:, k);
    yk = y == k;
    j(1,k) = sum(((-yk' * log(h) - (1 - yk)' * log(1 - h))));
end
...
reg1 = zeros(size(Theta1_grad));
reg1(:, 2:end) = lambda/m * Theta1(:,2:end);
Theta1_grad = 1/m * Delta_1 + reg1;
reg2 = zeros(size(Theta2_grad));
reg2(:, 2:end) = lambda/m * Theta2(:, 2:end);
